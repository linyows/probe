...
helloのステップでは`echo`を指定します。`with`に渡した値は`res`から参照できます。
...
このアクション固有のパラメータはありません。`with`に渡した内容はそのまま`res`に返るため、計算した値をoutputsとして公開したいときに便利です。
