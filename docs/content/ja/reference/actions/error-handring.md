...
失敗を無視するためのスイッチはありません。ワークフローを失敗させたくないチェックは、`test`で判定せず結果をoutputsとして記録します。
...
`retry`のパラメータは`max_attempts`（必須、1以上）、`interval`、`initial_delay`です。`max_attempts`の上限は環境変数`PROBE_MAX_ATTEMPTS`（既定10000）で変更できます。

## 後続の分岐

他のジョブが失敗したかどうかで分岐する手段はありません。ジョブが失敗すると後続はスキップされるためです。結果に応じて処理を変えたい場合は、結果をoutputsとして公開し、後続のジョブが`skipif`で判断します。
