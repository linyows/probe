...
他のジョブが失敗したかどうかで分岐する手段はありません。ジョブが失敗すると、それに依存するジョブはまとめてスキップされます。結果に応じて処理を変えたい場合は、結果を outputs として公開し、後続のジョブが `skipif` で判断します。
...
1. **[実行モデル](/ja/guide/concepts/execution-model)** - Probe がワークフローを実行する方法を学ぶ
