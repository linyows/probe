...
Probe 固有の [`parse_json`](./probe#parse-json)、[`encode_base64`](./probe#encode-base64)、[`decode_base64`](./probe#decode-base64) は同じ用途に使えます。
こちらは引数の型と長さを検証し、Probe のエラーメッセージを返します。

JSON の比較には [`match_json`](./probe#match-json) と [`diff_json`](./probe#diff-json) があります。
