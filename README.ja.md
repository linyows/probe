...
- **プラグインアーキテクチャ**: HTTP、データベース、ブラウザ、Shell、SSH、SMTP、IMAP、Helloアクションが組み込まれており、拡張可能
...
### Common Workflow Language（CWL）やWorkflow Description Language（WDL）とどう違う？

CWLやWDLは科学計算やバイオインフォマティクス分野での複雑なデータ処理パイプライン向けに設計された強力なワークフロー言語ですが、ProbeはWebベースのユースケースに特化した異なるアプローチを取っています。

**主な違い：**

- **対象ユースケース**: CWLはDockerベースの分散実行による科学データ処理に焦点を当て、WDLは強い型システムとクラウドベースの分散コンピューティングによるゲノミクスパイプラインを対象としています。Probeは、API監視、テスト自動化、運用タスクに特化して設計されています。
- **インターネットプロトコルへの特化**: Probeは、HTTP、SMTP、SSH、IMAP、ブラウザ自動化の組み込みサポートを提供し、Webユースケースに特化しています。この特化により、インターネットプロトコルベースのタスクを最小限の設定でワークフローとして定義・実行できます。
- **シンプルさと柔軟性**: ProbeはCWLやWDLと比較して汎用的な柔軟性は劣りますが、これは意図的な設計選択です。Webとインターネットプロトコルにフォーカスすることで、対象領域での生産性を大幅に向上させています。Protocol Buffersを使用したプラグインベースのアーキテクチャにより、必要に応じて機能拡張も可能です。
- **実行モデル**: 分散コンピューティング環境向けに設計されたCWLやWDLとは異なり、Probeは軽量でローカル実行可能、かつリアルタイム監視機能に最適化されています。
- **構文**: Probeは、CWLやWDLに見られる複雑な型システムではなく、直感的なYAMLベースの設定を使用し、運用チームやWebサービスに携わる開発者にとってアクセスしやすくなっています。

Probeの価値は、必要に応じてカスタムアクションによる機能拡張の能力を維持しながら、Webベースの自動化タスクに最も効率的なパスを提供することにあります。
