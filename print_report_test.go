...
func TestPrinter_printRepeatStepFromResult(t *testing.T) {
	printer := NewPrinter(false, []string{"job1"})

	// Test with skipped repeat step
	stepResult := StepResult{
		Index:   0,
		Name:    "Test Step (SKIPPED)",
	...
	printer.printRepeatStepFromResult("job1", stepResult)
	...
	// Should remove (SKIPPED) suffix from step name
	if !strings.Contains(outputStr, "Test Step (repeating 2 times)") {
		t.Error("Output should contain step name without SKIPPED suffix")
	}
	if !strings.Contains(outputStr, "2/2 completed (no test)") {
		t.Error("Output should contain completion ratio for no-test repeat")
	}
}
