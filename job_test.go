...
func TestJobScheduler_ConcurrentAccess(t *testing.T) {
	...
	// Goroutine 1: Check if job can run
	go func() {
		for i := 0; i < 100; i++ {
	...
	// Goroutine 2: Increment repeat counter
	go func() {
		for i := 0; i < 100; i++ {
	...
	// Goroutine 3: Check repeat status
	go func() {
		for i := 0; i < 100; i++ {
	...
	if current != 100 {
		t.Errorf("Expected counter to be 100, got %d", current)
	}
}